...
class HS_35(object):
    ...
    def __init__(self):
        ...
        # use constraints from the problem as the bounds for lower
    ...
    def _Objective(self, x1, x2, x3):
        func = 9.0 - 8.0*x1 - 6.0*x2 - 4.0*x3 + 2.0*x1*x1 + 2.0*x2*x2 + x3*x3 + 2.0*x1*x2 + 2.0*x1*x3
        return -func # paper assumes min - this makes it max

    def _Constraint1(self, x1, x2, x3):
        cons = 3.0 - x1 - x2 - 2.0*x3
        return cons
